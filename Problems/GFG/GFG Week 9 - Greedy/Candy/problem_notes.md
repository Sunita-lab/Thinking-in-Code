...
## Why Two Passes Are Sufficient — No Third Pass Needed

Each element has exactly **two neighbors** — left and right. Pass 1 satisfies the left neighbor constraint for every element in the array. Pass 2 satisfies the right neighbor constraint for every element. No element has a third neighbor, so no third pass is needed.

The guard condition in Pass 2 (`ans[i] <= ans[i+1]`) ensures Pass 2 never breaks what Pass 1 fixed — both constraints remain satisfied simultaneously after Pass 2 completes. There is nothing left unsatisfied.

---

## Why "Only Increase, Never Decrease" — Constraint Type Insight

The constraint is **"at least 1 candy"** — a lower bound, no upper bound. This means:
- We can always give more candy than required — it won't violate any constraint
- We can never give less than 1 — that's the floor
- We can never give less than a neighbor when our rating is higher — that's the comparison constraint

Because there's no upper bound, the greedy rule simplifies to: **only increase when necessary, never decrease.** If a neighbor already has enough candy relative to us, we leave our value unchanged. If they don't, we bump up by 1.

**What if there was an upper bound too?** For example, "adjacent difference must be exactly 1, not more." Then increasing one element might force decreasing another — the two constraints would conflict, and greedy's simple "only increase" rule would break. A more complex approach (constraint propagation or DP) would be needed.

**What if there was no lower bound (0 candies allowed)?** Same approach would work — greedy rule unchanged. Minimum totals would just be slightly lower since starting point is 0 instead of 1.

The lower-bound-only constraint is what makes the greedy so clean here — one direction (increase) is always safe, the other direction (decrease) is never needed.

---

## Proof That This Gives Minimum Candies

**Claim:** The value assigned to each position by this approach is the smallest possible value that satisfies both constraints.

**Proof by contradiction:** Suppose we could reduce `ans[i]` by 1 and still satisfy all constraints. Three cases:

**Case 1 — `arr[i] > arr[i-1]`:**
Pass 1 set `ans[i] = ans[i-1] + 1`. If we reduce to `ans[i] - 1 = ans[i-1]`, then `ans[i] = ans[i-1]` — but `arr[i] > arr[i-1]` requires strictly more candy. Left constraint violated. ❌

**Case 2 — `arr[i] > arr[i+1]`:**
Pass 2 set `ans[i] = ans[i+1] + 1`. If we reduce to `ans[i] - 1 = ans[i+1]`, right constraint is violated. ❌

**Case 3 — Neither neighbor has lower rating:**
`ans[i] = 1` — already at the minimum possible value (lower bound). Cannot reduce further. ❌

All three cases lead to contradiction — no position can be reduced. Therefore every assigned value is already the minimum. The total sum is minimum. ✅

**Intuition:** At every position, we give exactly one more than the relevant neighbor — no more, no less. This is precisely the minimum required by the constraint.
